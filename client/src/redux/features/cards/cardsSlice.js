...
export const cardSlice = createSlice({
    ...
    reducers: {
        deleteTemperamentFilter: (state, action) => {
            //Aplied filters
            state.filteredBy.searchedTemperaments =
                state.filteredBy.searchedTemperaments.filter(
                    (el) => el.name !== action.payload.name
                );
            let filtered = state.dogs.filter((item) =>
                matchFilters(
                    item,
                    state.filteredBy.searchedTemperaments,
                    state.filteredBy.origin
                )
            );
            state.filtered = filtered;
            //Set ordered
            const aux = orderCards(
                state.filtered,
                state.orderBy.parameter,
                state.orderBy.asc
            );
            state.ordered = aux;
            //Set pagination
        ...
        changePage: (state, action) => {
            const { displayed, total, index } = getPagination(
                state.ordered,
                action.payload
            );
            state.pagination.index = index;
...
export const {
    addCard,
    changePage,
    addTemperamentFilter,
    filterOrigin,
    deleteTemperamentFilter,
    orderSort,
    order,
} = cardSlice.actions;
